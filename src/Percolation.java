...
public class Percolation {
    ...
    private WeightedQuickUnionUF backChecked;
    ...
    public Percolation(int n) {
        ...
        backChecked = new WeightedQuickUnionUF(n * n + 1);
    ...
    public void open(int row, int col) {
        ...
        if (row == 1) {
            ...
            backChecked.union(getFlattenId(row, col), startPoint);
        }
        ...
        if (row > 1 && isOpen(row - 1, col)) {
            ...
            backChecked.union(getFlattenId(row - 1, col), getFlattenId(row, col));
        }
        ...
        if (col > 1 && isOpen(row, col - 1)) {
            ...
            backChecked.union(getFlattenId(row, col - 1), getFlattenId(row, col));
        }
    ...
    public boolean isFull(int row, int col) {
        if (row < 1 || row > gridSize || col < 1 || col > gridSize) throw new IndexOutOfBoundsException();
        return backChecked.connected(getFlattenId(row, col), startPoint);
    }
